...
calender._setupHelper = function(dates) {
    ...
    _.each(dates, function(date) {
        ...
        var expectedLocation = _.sortedIndex(bundleList, date.start, function(obj) {
            ...
            startBundle = null,
            endBundle = null,
            addToExistingBundle = function(bundle) {
            ...
            if (startBundle && endBundle) {
                ...
                if (startBundle === endBundle) {
                    ...
                    startBundle.each(function(model) {
                        model.updateShared(startBundle.length + 1);
                    });
                    startBundle.add(
                        new Calendar.EventItem(_.extend(obj, {
                            sharedItems: startBundle.length + 1,
                            itemNumber: startBundle.length
                        }))
                    );
                } else {
                ...
                endBundle.add(
                    new Calendar.EventItem(_.extend(obj, {
                        sharedItems: endBundle.length + 1,
                        itemNumber: endBundle.length
                    }))
                );
            } else {
